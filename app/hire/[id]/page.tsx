...
export default function HirePage() {
  ...
  const saveBooking = async (amount: number, ref: string | null, type: string, date: string | null, courseId: string | null) => {
    ...
    if (res.ok) {
      alert(type === 'trial' ? "Trial Booked Successfully!" : "Booking Successful!");
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="p-8 md:w-7/12 bg-white">
            ...
            <div className="flex bg-gray-100 p-1.5 rounded-xl mb-8">
              <button onClick={() => setBookingType('paid')} className={`flex-1 py-3 rounded-lg font-bold text-sm transition ${bookingType === 'paid' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>Standard</button>
              <button onClick={() => setBookingType('course')} className={`flex-1 py-3 rounded-lg font-bold text-sm transition ${bookingType === 'course' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500 hover:text-gray-700'}`}>Courses</button>
              <button onClick={() => setBookingType('trial')} className={`flex-1 py-3 rounded-lg font-bold text-sm transition ${bookingType === 'trial' ? 'bg-white shadow-sm text-green-600' : 'text-gray-500 hover:text-gray-700'}`}>Free Trial</button>
            </div>
                ...
                {courses.length === 0 ? (
                  ...
                  <>
                    <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                      {courses.map(c => (
                        <div key={c.id} onClick={() => setSelectedCourse(c)} className={`p-4 border rounded-xl cursor-pointer transition-all ${selectedCourse?.id === c.id ? 'border-purple-600 bg-purple-50 shadow-sm ring-1 ring-purple-600' : 'border-gray-200 hover:border-gray-300'}`}>
                    ...
                    {selectedCourse ? (
                      ...
                      <button disabled className="w-full bg-gray-200 text-gray-400 font-bold py-4 rounded-xl cursor-not-allowed">Select a course above</button>
                    )}
                  </>
                )}
              </div>
            ...
            {bookingType === 'trial' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                ...
                <div>
                  ...
                  <input aria-label="Select Trial Date" type="datetime-local" className="w-full p-4 border border-gray-300 rounded-xl bg-gray-50 outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500 transition" onChange={(e) => setTrialDate(e.target.value)} />
                </div>
                <button onClick={handleTrialSubmit} className="w-full bg-green-600 text-white font-bold py-4 rounded-xl hover:bg-green-700 transition shadow-lg shadow-green-600/20">Confirm Free Trial</button>
              </div>
            )}
