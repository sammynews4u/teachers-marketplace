...
export default function TeacherDashboard() {
  ...
  const handleStartChat = async (studentId: string) => {
    if (!confirm("Start conversation?")) return;
  ...
  // ACCOUNTPE PAYMENT FOR PACKAGES
                  ...
                  {courses.map(course => (
                    <div key={course.id} className="bg-white p-6 rounded-xl shadow-sm border flex justify-between items-center">
                      <div className="flex gap-4 items-center">
                          {course.image && <img src={course.image} className="w-16 h-16 rounded object-cover bg-gray-100"/>}
                      ...
                      <div className="flex gap-2">
                        {/* THIS BUTTON NOW LINKS TO THE BUILDER */}
